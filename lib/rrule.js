...
var dateutil = {
    ...
    cloneDates: function(dates) {
        ...
        for (var i = 0; i < dates.length; i++) {
            var _dates = dates[i];
            if ( _dates instanceof Object ) {
                clones.push({
                    start: dateutil.clone(_dates.start),
                    end: dateutil.clone(_dates.end)
                });
            } else {
...
var RRule = function(options, noCache) {
    ...
    if (!opts.dtend) {
        opts.dtend = new Date( opts.dtstart.getTime() )
    }
            ...
            if (plb(bysetpos) && plb(timeset)) {
                ...
                for (j = 0; j < poslist.length; j++) {
                    ...
                    if (until && res > until) {
                        ...
                        var post_date = new Date( diff > 0 ? res.getTime() + diff : res.getTime() );
                        if (!iterResult.accept(res, post_date)) {
                            return iterResult.getValue();
                        }
                        if (count) {
                            --count;
...
IterResult.prototype = {
     ...
     *                   false if we're done.
     */
    accept: function(date, post_date) {
        ...
        return this.add(date, post_date);
     ...
     */
    add: function(date, post_date) {
        if ( post_date ) {
            this._result.push({start: date, end: post_date});
        } else {
